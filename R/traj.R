...
## packages
lib <- c("opentraj", "doParallel", "Rsenal", "reshape2", "grid", "latticeExtra")
Orcs::loadPkgs(lib)
  ...
  for (j in 1:6) {
    ...
    lst <- foreach(i = 1:nrow(sbs), .packages = lib) %dopar% {
      ...
      spplot(sln, scales = list(draw = TRUE, cex = .7), 
        ...
        layer(sp.text(c(59, 1), lbl, font = 1, cex = .6), 
              data = list(lbl = lbl))
    }
  
    img <- suppressWarnings(latticeCombineGrid(lst, layout = c(1, 3)))
    ofl <- paste0(rsv, "/data/bale/HYSPLIT/results/vis/iso_", h, "_",
                  paste0(j, "d"), ".png")
